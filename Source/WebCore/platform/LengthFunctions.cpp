...
{
    return valueForLengthWithLazyMaximum<LayoutUnit, LayoutUnit>(length, [&] ALWAYS_INLINE_LAMBDA { return maximumValue; }, zoom);
}

float floatValueForLength(const Length& length, float maximumValue, float zoom)
{
    return valueForLengthWithLazyMaximum<float, float>(length, [&] ALWAYS_INLINE_LAMBDA { return maximumValue; }, zoom);
}
