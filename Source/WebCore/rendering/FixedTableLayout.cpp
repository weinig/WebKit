...
float FixedTableLayout::calcWidthArray()
{
    ...
    for (RenderTableCol* col = m_table->firstColumn(); col; col = col->nextColumn()) {
        ...
        if (auto fixedColStyleLogicalWidth = colStyleLogicalWidth.tryFixed(); fixedColStyleLogicalWidth && fixedColStyleLogicalWidth->evaluate(col->style()) > 0)
            effectiveColWidth = fixedColStyleLogicalWidth->evaluate(col->style());
        ...
        while (span) {
            ...
            if (auto fixedColStyleLogicalWidth = colStyleLogicalWidth.tryFixed(); fixedColStyleLogicalWidth && fixedColStyleLogicalWidth->evaluate(col->style()) > 0) {
                m_width[currentEffectiveColumn] = Style::PreferredSize::Fixed { fixedColStyleLogicalWidth->evaluate(col->style()) * spanInCurrentEffectiveColumn };
    ...
    for (RenderTableCell* cell = firstRow->firstCell(); cell; cell = cell->nextCell()) {
        ...
        if (auto fixedLogicalWidth = logicalWidth.tryFixed(); fixedLogicalWidth && fixedLogicalWidth->evaluate(1.0f /*FIXME FIND STYLE*/) > 0) {
        ...
        while (usedSpan < span && currentColumn < nEffCols) {
            ...
            if (m_width[currentColumn].isAuto() && !logicalWidth.isAuto()) {
                if (auto fixedLogicalWidth = logicalWidth.tryFixed())
                    m_width[currentColumn] = Style::PreferredSize::Fixed { fixedLogicalWidth->evaluate(1.0f /*FIXME FIND STYLE*/) * eSpan / span };
...
void FixedTableLayout::applyPreferredLogicalWidthQuirks(LayoutUnit& minWidth, LayoutUnit& maxWidth) const
{
    ...
    if (auto fixedTableLogicalWidth = tableLogicalWidth.tryFixed(); fixedTableLogicalWidth && fixedTableLogicalWidth->evaluate(m_table->style()) > 0)
        minWidth = maxWidth = std::max(minWidth, LayoutUnit(fixedTableLogicalWidth->evaluate(m_table->style())) - m_table->bordersPaddingAndSpacingInRowDirection());
...
void FixedTableLayout::layout()
{
    ...
    for (unsigned i = 0; i < nEffCols; i++) {
        if (auto fixedWidth = m_width[i].tryFixed()) {
            calcWidth[i] = fixedWidth->evaluate(1.0f /*FIXME FIND STYLE*/);
