...
LayoutSize RenderBoxModelObject::relativePositionOffset() const
{
    ...
    if (topFixed && leftFixed && bottom.isAuto() && right.isAuto() && containingBlock->writingMode().isAnyLeftToRight()) {
        offset.expand(leftFixed->evaluate(style), topFixed->evaluate(style));
        return offset;
    }
    ...
    if (!left.isAuto() || !right.isAuto()) {
        ...
        if (!left.isAuto()) {
            if (!right.isAuto() && !containingBlock->writingMode().isAnyLeftToRight())
                offset.setWidth(-Style::evaluate(right, !right.isFixed() ? availableWidth() : 0_lu, style));
            else
                offset.expand(Style::evaluate(left, !left.isFixed() ? availableWidth() : 0_lu, style), 0_lu);
        } else if (!right.isAuto())
            offset.expand(-Style::evaluate(right, !right.isFixed() ? availableWidth() : 0_lu, style), 0_lu);
    }
    ...
    if (!top.isAuto() && (!top.isPercentOrCalculated() || containingBlockHasDefiniteHeight)) {
        ...
        offset.expand(0_lu, Style::evaluate(top, !top.isFixed() ? availableHeight() : 0_lu, style));
        ...
        offset.expand(0_lu, -Style::evaluate(bottom, !bottom.isFixed() ? availableHeight() : 0_lu, style));
    }
    return offset;
}
...
void RenderBoxModelObject::computeStickyPositionConstraints(StickyPositionViewportConstraints& constraints, const FloatRect& constrainingRect) const
{
    ...
    if (!style().left().isAuto()) {
        constraints.setLeftOffset(Style::evaluate(style().left(), constrainingRect.width(), style()));
        constraints.addAnchorEdge(ViewportConstraints::AnchorEdgeLeft);
    }

    if (!style().right().isAuto()) {
        constraints.setRightOffset(Style::evaluate(style().right(), constrainingRect.width(), style()));
        constraints.addAnchorEdge(ViewportConstraints::AnchorEdgeRight);
    }

    if (!style().top().isAuto()) {
        constraints.setTopOffset(Style::evaluate(style().top(), constrainingRect.height(), style()));
        constraints.addAnchorEdge(ViewportConstraints::AnchorEdgeTop);
    }

    if (!style().bottom().isAuto()) {
        constraints.setBottomOffset(Style::evaluate(style().bottom(), constrainingRect.height(), style()));
